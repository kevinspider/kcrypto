...
def aes_key_schedule(master_key: bytes, nk: int) -> list:
    ...
    round_keys = [[0] * 4 for _i in range(round_keys_len)]  # debug [ [x,x,x,x] * 44 ] 二维数组
...
# debug 入口函数
def aes_encrypt(message: bytes, master_key: bytes, master_iv: bytes | None = None, mode: AesMode = AesMode.ECB,
