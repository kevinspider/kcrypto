import struct

SBOX = (
    ,
)

RCON = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36,
)  # 11个 0x0 用不到占位

key_len_loop_num = {
    16: 10,  # AES-128
    24: 12,  # AES-192
    32: 14  # AES-256
}


def shift_left(array, num):
    return array[num:] + array[:num]


def g(array, index):
    # 1. Left Rotate
    array = shift_left(array, 1)
    # 2. Byte substitution using SBOX
    array = [SBOX[i] for i in array]
    # 3. XOR with RCON
    array[0] ^= RCON[index]
    return array


def xor_array(array1, array2):
    return [array1[i] ^ array2[i] for i in range(len(array1))]


def aes_key_schedule(master_key: bytes):
    assert len(master_key) in key_len_loop_num

    loop_num = key_len_loop_num[len(master_key)]
    round_keys_len = (loop_num + 1) * 4
    round_keys = [[0] * 4 for _ in range(round_keys_len)]
    nk = len(master_key) // 4

    # Initialize round keys from master key
    for i in range(nk):
        round_keys[i] = list(struct.unpack(">4B", master_key[i * 4: (i + 1) * 4]))

    # Generate the round keys
    for i in range(nk, round_keys_len):
        if i % nk == 0:
            round_keys[i] = xor_array(round_keys[i - nk], g(round_keys[i - 1], i // nk - 1))
        else:
            round_keys[i] = xor_array(round_keys[i - 1], round_keys[i - nk])

    show_round_keys(round_keys, loop_num)
    return round_keys


def show_round_keys(round_kyes: list[list], loop_num: int):
    kList = [[] for i in range(loop_num + 1)]
    for i in range(len(round_kyes)):
        kList[i // 4] += round_kyes[i]
    for i in range(len(kList)):
        print("K%02d: " % i + "".join("%02X" % k for k in kList[i]))


if __name__ == '__main__':
    master_key = bytes.fromhex("1122334455667788112233445566778811223344556677881122334455667788")
    aes_key_schedule(master_key)